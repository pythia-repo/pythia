#include "backends/pythia/pythia/test/small-step/util.h"
...
boost::optional<const PythiaTestCase> createSmallStepExprTest(const std::string& hdrFields,
...
V1Switch(parse(), verifyChecksum(), mau(), mau(), computeChecksum(), deparse()) main;)");
    ...
    return PythiaTestCase::create_16("bmv2", "v1model", source);
}
