...
#include "backends/pythia/pythia/test/z3-solver/accessor.h"

namespace Test {

using Pythia::Model;
using Pythia::StateVariable;
using Pythia::Z3Solver;
using Pythia::Z3SolverAccessor;
using Pythia::P4Pythia::PythiaTarget;
using Value = IR::Literal;

class Z3SolverTest : public PythiaTest {
 protected:
    virtual void SetUp() {
        opLss = nullptr;
      ...
      V1Switch(parse(), verifyChecksum(), mau(), mau(), computeChecksum(), deparse()) main;)");
        ...
        const auto test = PythiaTestCase::create_16("bmv2", "v1model", source);
        ...
        const auto* progInfo = PythiaTarget::initProgram(test->program);
        if (progInfo == nullptr) {
            return;
        }
...
TEST_F(Z3SolverTest, Assertion2Model) {
    ...
    std::vector<const Pythia::Constraint*> asserts;
