...
std::vector<Continuation::Command> EBPFProgramInfo::processDeclaration(
    ...
    if (const auto* p4parser = declType->to<IR::P4Parser>()) {
        ...
        PYTHIA_UNIMPLEMENTED("Constructed type %s of type %s not supported.", declType,
    ...
    // After some specific pipelines (filter), we have to append the remaining packet
    ...
    if ((archMember->blockName == "filter")) {
        ...
        auto* emitBlock = new IR::BlockStatement();
        emitBlock->push_back(
            new IR::MethodCallStatement(IRUtils::generateInternalMethodCall("emit_ebpf_hdrs", {})));
        emitBlock->push_back(new IR::MethodCallStatement(
            IRUtils::generateInternalMethodCall("prepend_emit_buffer", {})));
        const auto* dropCheck = new IR::IfStatement(dropIsActive(), dropStmt, emitBlock);
...
const IR::Member* EBPFProgramInfo::getTargetInputPortVar() const {
    return new IR::Member(IRUtils::getBitType(PythiaTarget::getPortNumWidth_bits()),
