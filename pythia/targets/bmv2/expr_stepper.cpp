...
#include "backends/pythia/pythia/targets/bmv2/table_stepper.h"
    ...
    static const ExternMethodImpls EXTERN_METHOD_IMPLS({
         ...
         [](const IR::MethodCallExpression* /*call*/, const IR::Expression* /*receiver*/,
            ...
            const ExecutionState& state, SmallStepEvaluator::Result& result) {
             ...
             const auto* nineBitType =
                 IRUtils::getBitType(BMv2_V1ModelPythiaTarget::getPortNumWidth_bits());
             const auto* metadataLabel = args->at(0)->expression;
             if (!(metadataLabel->is<IR::Member>() || metadataLabel->is<IR::PathExpression>())) {
                 PYTHIA_UNIMPLEMENTED("Drop input %1% of type %2% not supported", metadataLabel,
                                       metadataLabel->type);
             }
        ...
        {"*method.random",
         ...
         [this](const IR::MethodCallExpression* /*call*/, const IR::Expression* /*receiver*/,
             ...
             if (fieldRef == nullptr) {
                 PYTHIA_UNIMPLEMENTED("Random output %1% of type %2% not supported", resultField,
                                       resultField->type);
             }
        ...
        {"*method.hash",
         ...
         [this](const IR::MethodCallExpression* call, const IR::Expression* receiver, IR::ID& name,
             ...
             if (hashOutput->type->is<IR::Type_Bits>()) {
                 ...
                 if (fieldRef == nullptr) {
                     PYTHIA_UNIMPLEMENTED("Hash output %1% of type %2% not supported", hashOutput,
                                           hashOutput->type);
                 }
                 if (argsAreTainted) {
                     nextState->set(fieldRef,
        ...
        {"register.read",
         ...
         [this](const IR::MethodCallExpression* call, const IR::Expression* receiver,
             ...
             if (readOutput->type->is<IR::Type_Bits>()) {
                 ...
                 PYTHIA_UNIMPLEMENTED("Read extern output %1% of type %2% not supported",
        ...
        {"*method.clone_preserving_field_list",
         ...
         [this](const IR::MethodCallExpression* call, const IR::Expression* /*receiver*/,
             ...
             if (cloneType == BMv2Constants::CLONE_TYPE_I2E) {
                 ...
                 const auto* rndConst =
                     IRUtils::getRandConstantForWidth(PythiaTarget::getPortNumWidth_bits());
             ...
             PYTHIA_UNIMPLEMENTED("Unsupported clone type %1%.", cloneType);
         }},
        /* ======================================================================================
        ...
        {"*method.update_checksum",
         ...
         [this](const IR::MethodCallExpression* call, const IR::Expression* /*receiver*/,
             ...
             if (!(checksumVar->is<IR::Member>() || checksumVar->is<IR::PathExpression>())) {
                 PYTHIA_UNIMPLEMENTED("Checksum input %1% of type %2% not supported", checksumVar,
