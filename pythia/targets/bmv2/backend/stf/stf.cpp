#include "backends/pythia/pythia/targets/bmv2/backend/stf/stf.h"
...
void STF::outputTest(const TestSpec* testSpec, cstring selectedBranches, size_t testIdx,
    ...
    comment(stfFile, "p4pythia seed: " + boost::lexical_cast<std::string>(seed));
    if (selectedBranches != "") {
        comment(stfFile, selectedBranches);
    }
    comment(stfFile, "Date generated: " + PythiaUtils::getTimeStamp());
