#include "backends/pythia/pythia/targets/bmv2/concolic.h"
...
#include "backends/pythia/pythia/targets/bmv2/contrib/bmv2_hash/calculations.h"
...
big_int Bmv2Concolic::computeChecksum(const std::vector<const IR::Expression*>& exprList,
    ...
    switch (algo) {
        ...
        default:
            PYTHIA_UNIMPLEMENTED("Algorithm %1% not implemented for hash.", algo);
    }
    ...
    {"*method_hash",
     ...
     [](cstring /*concolicMethodName*/, const IR::ConcolicVariable* var,
        ...
        ConcolicVariableMap* resolvedConcolicVariables) {
         ...
         if (exprList.empty()) {
             PYTHIA_UNIMPLEMENTED("Data input is empty. This case is not implemented.");
         }
         ...
         if (const auto* checksumVarType = checksumVar->type->to<IR::Type_Bits>()) {
             ...
             PYTHIA_UNIMPLEMENTED("Checksum output %1% of type %2% not supported", checksumVar,
                                   checksumVar->type);
         }
