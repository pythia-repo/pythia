...
#     the logic of the target test. "pythia_add_test_list" includes this file and then
#     executes the logic defined "pythia_add_test_with_args".
...
#
macro(pythia_add_test_list template_file tag driver tests xfail target arch std enable_runner)
...
# to pythia_add_test_list
# Arguments:
#   - template_file The path to the extension/target file that implements
#     the logic of the target test. "pythia_add_tests" includes this file and then
#     executes the logic defined "pythia_add_test_with_args".
#   - tag is a label for the set of test suite where this test belongs
#     (for example, p4ctest)
...
# The macro generates the test files in a directory prefixed by tag.
#
macro(pythia_add_tests template_file tag driver testsuites xfails target arch std enable_runner)
  set(__tests "")
  set(__xfails "")
  p4c_find_test_names("${testsuites}" __tests)
  p4c_find_test_names("${xfails}" __xfails)
  pythia_add_test_list(
    ${template_file}
    ${tag} ${driver} "${__tests}" "${__xfails}"
    ${target} ${arch} ${std} ${enable_runner} "${ARGN}"
  )
endmacro(pythia_add_tests)

# Used to add a label to the test like xfail
macro(pythia_add_test_label tag newLabel testname)
...
endmacro(pythia_add_test_label)
...
macro(pythia_add_xfail_reason tag reason)
  set(__tests "${ARGN}")
