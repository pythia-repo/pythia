...
PythiaTarget::PythiaTarget(std::string deviceName, std::string archName)
    : Target("pythia", std::move(deviceName), std::move(archName)) {}

const ProgramInfo* PythiaTarget::initProgram_impl(const IR::P4Program* program) const {
...
const PythiaTarget& PythiaTarget::get() { return Target::get<PythiaTarget>("pythia"); }

void PythiaTarget::argumentsToTypeDeclarations(
