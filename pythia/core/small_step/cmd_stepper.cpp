...
#include "backends/pythia/common/compiler/hs_index_simplify.h"
...
void CmdStepper::initializeBlockParams(const IR::Type_Declaration* typeDecl,
    ...
    if (const auto* p4parser = typeDecl->to<IR::P4Parser>()) {
        ...
        nextState->pushNamespace(p4parser);
    } else if (const auto* p4control = typeDecl->to<IR::P4Control>()) {
        params = p4control->getApplyParameters();
        nextState->pushNamespace(p4control);
    } else {
        PYTHIA_UNIMPLEMENTED("Constructed type %s of type %s not supported.", typeDecl,
...
bool CmdStepper::preorder(const IR::AssignmentStatement* assign) {
    ...
    if (const auto* structType = leftType->to<IR::Type_StructLike>()) {
        ...
        PYTHIA_UNIMPLEMENTED("Unsupported assign type %1% node: %2%", leftType,
...
bool CmdStepper::preorder(const IR::Declaration_Variable* decl) {
    ...
    if (decl->initializer != nullptr) {
        PYTHIA_UNIMPLEMENTED("Unsupported initializer %s for declaration variable.",
                              decl->initializer);
    }
    ...
    if (const auto* structType = declType->to<IR::Type_StructLike>()) {
        ...
        PYTHIA_UNIMPLEMENTED("Unsupported declaration type %1% node: %2%", declType,
...
bool CmdStepper::preorder(const IR::P4Program* /*program*/) {
    ...
    auto pktSize = PythiaOptions::get().packetSize;
    if (pktSize != 0) {
    ...
    if (PythiaOptions::get().inputPacketOnly) {
