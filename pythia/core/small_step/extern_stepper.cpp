         ...
         [](const IR::MethodCallExpression* /*call*/, const IR::Expression* /*receiver*/,
            ...
            const ExecutionState& state, SmallStepEvaluator::Result& result) {
             ...
             if (!(prependVar->is<IR::Member>() || prependVar->is<IR::PathExpression>() ||
                 ...
                 PYTHIA_UNIMPLEMENTED("Prepend input %1% of type %2% not supported", prependVar,
                                       prependVar->type);
             }
         ...
         [](const IR::MethodCallExpression* /*call*/, const IR::Expression* /*receiver*/,
            ...
            const ExecutionState& state, SmallStepEvaluator::Result& result) {
             ...
             if (!(appendVar->is<IR::Member>() || appendVar->is<IR::PathExpression>() ||
                 ...
                 PYTHIA_UNIMPLEMENTED("append input %1% of type %2% not supported", appendVar,
                                       appendVar->type);
             }
             auto* nextState = new ExecutionState(state);
        ...
        {"*.copy_in",
         ...
         [this](const IR::MethodCallExpression* /*call*/, const IR::Expression* /*receiver*/,
             ...
             if (!(globalRef->is<IR::Member>() || globalRef->is<IR::PathExpression>())) {
                 PYTHIA_UNIMPLEMENTED("Global input %1% of type %2% not supported", globalRef,
                                       globalRef->type);
             }

             const auto* paramRef = args->at(1)->expression;
             if (!(paramRef->is<IR::Member>() || paramRef->is<IR::PathExpression>())) {
                 PYTHIA_UNIMPLEMENTED("Param input %1% of type %2% not supported", paramRef,
                                       paramRef->type);
             }
             ...
             if (const auto* ts = assignType->to<IR::Type_StructLike>()) {
                 ...
                 if (const auto* paramPath = paramRef->to<IR::PathExpression>()) {
                     paramRef = nextState->convertPathExpr(paramPath);
                 }
         ...
         [](const IR::MethodCallExpression* /*call*/, const IR::Expression* /*receiver*/,
            ...
            const ExecutionState& state, SmallStepEvaluator::Result& result) {
             ...
             if (!emitOutput->is<IR::Member>()) {
                 PYTHIA_UNIMPLEMENTED("Emit input %1% of type %2% not supported", emitOutput,
                                       emitType);
             }
